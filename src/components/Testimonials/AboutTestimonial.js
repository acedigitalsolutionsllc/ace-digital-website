import React from "react";
import { Autoplay, Navigation, Pagination } from "swiper/modules";
import { Swiper, SwiperSlide } from "swiper/react";

export default function AboutTestimonial() {
  const swiperOptions = {
    modules: [Autoplay, Pagination, Navigation],
    slidesPerView: 1,
    spaceBetween: 30,
    loop: true,
    speed: 3000,
    autoplay: {
      delay: 2500,
    },
    pagination: {
      el: ".h3-testimonial-pagination",
      clickable: true,
    },
    navigation: {
      nextEl: ".h3-testimonial-next",
      prevEl: ".h3-testimonial-prev",
    },
  };
  
  return (
    <section className="pt-16 md:pt-[130px] pb-40 md:pb-[230px]">
      <div className="mx-auto theme-container">
        <div className="w-full">
          <div className="flex flex-col items-center">
            {/* Tag updated to Brand Green with Glow Border */}
            <span className="rounded-[30px] px-5 py-1 mb-5 !bg-[#6db305]/10 font-semibold !text-[#6db305] border !border-[#6db305]/20 shadow-[0_0_10px_rgba(109,179,5,0.15)] uppercase tracking-wider text-xs">
              Clients Feedback
            </span>
            <h2 className="text-24 sm:text-48 font-semibold text-main-black mb-[60px] text-center max-w-[653px] leading-tight">
              Our Customer Feedback
            </h2>
          </div>
        </div>
        
        <div className="flex flex-col items-center justify-center relative lg:px-[138px]">
          <div className="w-full h-full overflow-hidden h3-testimonial-slider">
            <Swiper {...swiperOptions}>
              
              {/* Slide 1 */}
              <SwiperSlide>
                <div className="flex flex-col justify-between">
                  <div>
                    <div className="flex flex-col items-center gap-[30px]">
                      
                      {/* Quote Icon 1 - Updated to Brand Blue with subtle glow */}
                      <svg
                        width="54"
                        height="40"
                        viewBox="0 0 54 40"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="drop-shadow-[0_0_8px_rgba(9,74,102,0.3)]"
                      >
                        <path
                          d="M49.5406 4.06287C47.1408 1.36698 44.2282 0 40.8838 0C37.8781 0 35.33 1.07033 33.3098 3.18112C31.3024 5.27842 30.2845 7.88345 30.2845 10.924C30.2845 13.7974 31.3127 16.3578 33.3404 18.5344C35.129 20.4545 37.3822 21.6759 40.0496 22.1736C39.5831 25.7101 36.4568 28.9111 30.7387 31.7003L29.709 32.2026L33.9307 39.9964L34.8837 39.5134C46.976 33.3839 53.1072 24.7214 53.1072 13.7664C53.1072 9.98687 51.9073 6.72226 49.5406 4.06287ZM34.8388 37.062L32.7236 33.1576C39.0843 29.8246 42.3069 25.8181 42.3069 21.2372V20.2564L41.3324 20.146C38.7077 19.849 36.6191 18.8322 34.9474 17.0374C33.2877 15.2557 32.4808 13.2562 32.4808 10.924C32.4808 8.43381 33.2711 6.39791 34.8964 4.69953C36.5086 3.01495 38.4672 2.19605 40.8839 2.19605C43.6124 2.19605 45.9074 3.28432 47.9 5.5229C49.9262 7.79943 50.9111 10.4958 50.9111 13.7663C50.9111 18.7872 49.4973 23.3202 46.7091 27.2392C44.0485 30.9785 40.0582 34.2797 34.8388 37.062Z"
                          fill="#094a66"
                        />
                        <path
                          d="M19.7738 4.0579C17.3473 1.36532 14.4226 0 11.0807 0C8.07213 0 5.53555 1.0723 3.54187 3.18703C1.5653 5.2835 0.563015 7.88657 0.563015 10.924C0.563015 13.7973 1.59113 16.3577 3.61863 18.5344C5.40351 20.4504 7.62964 21.6706 10.2474 22.1706C9.78658 25.7098 6.68627 28.9124 1.01401 31.7021L0 32.2006L4.1166 40L5.07906 39.5144C17.2262 33.3852 23.3853 24.7223 23.3853 13.7663C23.3852 9.98387 22.17 6.71749 19.7738 4.0579ZM5.04711 37.0583L2.98964 33.1599C9.30416 29.8257 12.5037 25.8182 12.5037 21.2371V20.2585L11.5314 20.1463C8.96052 19.8496 6.89766 18.8327 5.22542 17.0373C3.56573 15.2558 2.75906 13.2561 2.75906 10.924C2.75906 8.4306 3.53782 6.39252 5.13964 4.69362C6.72402 3.01308 8.6675 2.19605 11.0807 2.19605C13.8119 2.19605 16.122 3.28588 18.1422 5.52798C20.1925 7.80328 21.1892 10.4981 21.1892 13.7663C21.1892 18.7864 19.7692 23.3188 16.9683 27.2374C14.2963 30.9756 10.2888 34.2763 5.04711 37.0583Z"
                          fill="#094a66"
                        />
                      </svg>

                      <h1 className="text-18 sm:text-[35px] sm:leading-[55px] text-center text-main-black font-medium pt-5">
                        ACE Digital completely transformed our online presence. 
                        Their AI avatars created so much content for us 
                        that our leads doubled within the first month. Highly recommended!
                      </h1>
                      
                      <div className="flex items-center justify-center gap-5 mt-10">
                        <div className="flex min-w-12 w-12 sm:w-[60px] min-h-12 h-12 sm:h-[60px] rounded-full overflow-hidden border-2 border-[#e7e3fa]">
                          <img
                            className="object-cover w-full h-full"
                            src="/assets/images/about/testimonial-1.webp"
                            alt="Matthew C. Lansberry"
                          />
                        </div>
                        <div>
                          <h1 className="font-semibold text-20 text-main-black">
                            Matthew C. Lansberry,
                          </h1>
                          <p className="text-sm font-medium text-paragraph">
                            CEO & Founder
                          </p>
                        </div>
                      </div>
                      
                    </div>
                  </div>
                </div>
              </SwiperSlide>

              {/* Slide 2 */}
              <SwiperSlide>
                <div className="flex flex-col justify-between">
                  <div>
                    <div className="flex flex-col items-center gap-[30px]">
                      
                      {/* Quote Icon 2 - Updated to Brand Blue with subtle glow */}
                      <svg
                        width="54"
                        height="40"
                        viewBox="0 0 54 40"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="drop-shadow-[0_0_8px_rgba(9,74,102,0.3)]"
                      >
                        <path
                          d="M49.5406 4.06287C47.1408 1.36698 44.2282 0 40.8838 0C37.8781 0 35.33 1.07033 33.3098 3.18112C31.3024 5.27842 30.2845 7.88345 30.2845 10.924C30.2845 13.7974 31.3127 16.3578 33.3404 18.5344C35.129 20.4545 37.3822 21.6759 40.0496 22.1736C39.5831 25.7101 36.4568 28.9111 30.7387 31.7003L29.709 32.2026L33.9307 39.9964L34.8837 39.5134C46.976 33.3839 53.1072 24.7214 53.1072 13.7664C53.1072 9.98687 51.9073 6.72226 49.5406 4.06287ZM34.8388 37.062L32.7236 33.1576C39.0843 29.8246 42.3069 25.8181 42.3069 21.2372V20.2564L41.3324 20.146C38.7077 19.849 36.6191 18.8322 34.9474 17.0374C33.2877 15.2557 32.4808 13.2562 32.4808 10.924C32.4808 8.43381 33.2711 6.39791 34.8964 4.69953C36.5086 3.01495 38.4672 2.19605 40.8839 2.19605C43.6124 2.19605 45.9074 3.28432 47.9 5.5229C49.9262 7.79943 50.9111 10.4958 50.9111 13.7663C50.9111 18.7872 49.4973 23.3202 46.7091 27.2392C44.0485 30.9785 40.0582 34.2797 34.8388 37.062Z"
                          fill="#094a66"
                        />
                        <path
                          d="M19.7738 4.0579C17.3473 1.36532 14.4226 0 11.0807 0C8.07213 0 5.53555 1.0723 3.54187 3.18703C1.5653 5.2835 0.563015 7.88657 0.563015 10.924C0.563015 13.7973 1.59113 16.3577 3.61863 18.5344C5.40351 20.4504 7.62964 21.6706 10.2474 22.1706C9.78658 25.7098 6.68627 28.9124 1.01401 31.7021L0 32.2006L4.1166 40L5.07906 39.5144C17.2262 33.3852 23.3853 24.7223 23.3853 13.7663C23.3852 9.98387 22.17 6.71749 19.7738 4.0579ZM5.04711 37.0583L2.98964 33.1599C9.30416 29.8257 12.5037 25.8182 12.5037 21.2371V20.2585L11.5314 20.1463C8.96052 19.8496 6.89766 18.8327 5.22542 17.0373C3.56573 15.2558 2.75906 13.2561 2.75906 10.924C2.75906 8.4306 3.53782 6.39252 5.13964 4.69362C6.72402 3.01308 8.6675 2.19605 11.0807 2.19605C13.8119 2.19605 16.122 3.28588 18.1422 5.52798C20.1925 7.80328 21.1892 10.4981 21.1892 13.7663C21.1892 18.7864 19.7692 23.3188 16.9683 27.2374C14.2963 30.9756 10.2888 34.2763 5.04711 37.0583Z"
                          fill="#094a66"
                        />
                      </svg>

                      <h1 className="text-18 sm:text-[35px] sm:leading-[55px] text-center text-main-black font-medium pt-5">
                        The CGI product ads they created for us were jaw-dropping. 
                        It completely changed how our customers view our brand online. 
                        Exceptional quality and speed!
                      </h1>
                      
                      <div className="flex items-center justify-center gap-5 mt-10">
                        <div className="flex min-w-12 w-12 sm:w-[60px] min-h-12 h-12 sm:h-[60px] rounded-full overflow-hidden border-2 border-[#e7e3fa]">
                          <img
                            className="object-cover w-full h-full"
                            src="/assets/images/about/testimonial-1.webp"
                            alt="Sarah Jenkins"
                          />
                        </div>
                        <div>
                          <h1 className="font-semibold text-20 text-main-black">
                            Sarah Jenkins,
                          </h1>
                          <p className="text-sm font-medium text-paragraph">
                            Marketing Director
                          </p>
                        </div>
                      </div>
                      
                    </div>
                  </div>
                </div>
              </SwiperSlide>

            </Swiper>
          </div>
          
          {/* Slider Navigation Buttons */}
          <div className="absolute w-fit mx-auto flex items-center -bottom-[100px]">
            <div className="absolute flex justify-between w-full gap-8">
              
              {/* Prev Button - Updated to Brand Blue default, Brand Green on hover with Glow */}
              <button className="group h3-testimonial-prev border border-[#094a66]/20 w-[35px] h-[35px] rounded-full flex items-center justify-center bg-[#094a66]/10 overflow-hidden before:inline-block before:w-12 before:h-12 before:bg-[#6db305] relative before:absolute before:z-0 before:-right-12 hover:before:right-0 before:transition-all before:duration-300 hover:border-[#6db305] hover:shadow-[0_0_15px_rgba(109,179,5,0.5)]">
                <svg
                  className="relative z-10 pointer-events-none"
                  width="14"
                  height="12"
                  viewBox="0 0 12 10"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    className="stroke-[#094a66] group-hover:stroke-white transition-colors duration-300"
                    d="M4.75 9L1 5.25M1 5.25L4.75 1.5M1 5.25L11 5.25"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </button>
              
              {/* Next Button - Updated to Brand Blue default, Brand Green on hover with Glow */}
              <button className="group rotate-180 h3-testimonial-next border border-[#094a66]/20 w-[35px] h-[35px] rounded-full flex items-center justify-center bg-[#094a66]/10 overflow-hidden before:inline-block before:w-12 before:h-12 before:bg-[#6db305] relative before:absolute before:z-0 before:-right-12 hover:before:right-0 before:transition-all before:duration-300 hover:border-[#6db305] hover:shadow-[0_0_15px_rgba(109,179,5,0.5)]">
                <svg
                  className="relative z-10 pointer-events-none"
                  width="14"
                  height="12"
                  viewBox="0 0 12 10"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    className="stroke-[#094a66] group-hover:stroke-white transition-colors duration-300"
                    d="M4.75 9L1 5.25M1 5.25L4.75 1.5M1 5.25L11 5.25"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </button>

            </div>
            {/* Pagination Container */}
            <div className="px-16 h3-testimonial-pagination flex items-center gap-2"></div>
          </div>
          
        </div>
      </div>
    </section>
  );
}